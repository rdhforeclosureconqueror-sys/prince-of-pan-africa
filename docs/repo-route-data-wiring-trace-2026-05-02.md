...
## Executive summary
- Primary frontend runtime API origin is centralized at `API_BASE_URL` and defaults to `https://api.simbawaujamaa.com` in non-local hosts after Render verifies DNS and certificate; many calls use shared `api()` client with cookies forced on (`credentials: include`).
...
- `src/config.js`
  ...
  - `WS_BASE_URL`: env or the WebSocket equivalent of `API_BASE_URL` (`wss://api.simbawaujamaa.com` in production).
...
- Hardcoded backend/service origins found:
  ...
  - `https://prince-of-pan-africa-backend.onrender.com` (fallback backend only).
