...
export default function IntelligenceHealthMonitor() {
  ...
  const firstFailureIndex = hasDiagnosticResult ? (authoritativeFirstFailure ? dependencyLayers.findIndex((layer) => authoritativeFirstFailure.includes(layer)) : dependencyLayers.findIndex((layer) => { const match = layers.find((item) => (item.layer || "").includes(layer)); const evidence = runtimeEvidenceByLayer.get(layer); return !isRuntimeVerified(evidence) || match?.status === "FAIL" || match?.regression; })) : -1;
    ...
    <section className="cosmic-section intelligence-health-monitor" aria-labelledby="intelligence-health-title">
      ...
      {viewMode === "executive" && <section className="mission-control executive-mission-control" aria-label="Executive Brief">
        ...
        <div className="dashboard-grid executive-brief-grid">
          ...
          <article className="stat-card executive-priority"><h3>Root Cause</h3><p>{rootCauseSummary}</p><p>{rootCauseLayer ? `${operationalLayerName(rootCauseLayer)} is the first point of failure.` : "No first point of failure detected."}</p></article>
