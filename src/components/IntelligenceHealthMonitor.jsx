...
export default function IntelligenceHealthMonitor() {
  ...
  const warningVerificationCenter = safeObject(result?.warning_verification_center);
  const warningVerificationItems = asArray(warningVerificationCenter.items);
  const releaseReadinessChecklist = asArray(result?.release_readiness_checklist);
  const warningVerificationQueue = (warningVerificationItems.length ? warningVerificationItems : layers.filter((layer) => layer.status === "WARNING" || layer.display_status || (!hasActiveRegression && layer.regression)).slice(0, Math.max(3, warningCount || 0)).map((layer) => ({
    layer: layer.layer,
    warning_type: layer.diagnostic_category || "operational warning verification",
    current_status: layer.display_status || layer.status || "WARNING",
    why_it_is_still_a_warning: layer.plain_language_reason || "Warning evidence is incomplete; Mission Control will not mark release readiness ready until verification passes.",
    evidence_required_to_clear: "Runtime diagnostic evidence, expected-vs-actual baseline comparison, no-regression confirmation, and a clean rerun are required before this warning can be marked verified.",
    verification_action: ["Review runtime evidence", "Compare baseline", "Confirm no regression", "Rerun diagnostic", "Mark warning verified only if evidence passes"],
    owner: layer.owner || "Platform intelligence owner · evidence review",
    expected_cleared_state: `${operationalLayerName(layer.layer)} warning is verified with evidence and no longer blocks release readiness.`,
    release_readiness_impact: "Release Readiness remains below ready until this warning gate clears.",
    baseline_review_required: true, rerun_required: true, code_change_required: Boolean(layer.regression), missing_evidence: !asArray(layer.supporting_evidence).length,
    ...
    title: `Verify ${operationalLayerName(item.layer || `warning ${index + 1}`)} warning`,
    impact: item.release_readiness_impact || "Confirms the remaining warning is understood before release readiness is approved.",
    effort: item.rerun_required ? "Rerun diagnostic required" : "Evidence review",
    layers: operationalLayerName(item.layer || `warning ${index + 1}`),
    improvement: item.expected_cleared_state || "Warning-resolution evidence will be confirmed after verification.",
    owner: item.owner,
    evidence_required_to_clear: item.evidence_required_to_clear,
  ...
  const executiveWorkQueue = priorityQueue.slice(0, Math.max(3, Math.min(5, priorityQueue.length || 3))).map((action, index) => ({
    ...
    executableActions: ["Open Member Intelligence", "Launch Verification", "View Evidence", "Compare Baseline", "Mark Verified"],
  }));
  ...
  return (
    <section className="cosmic-section intelligence-health-monitor" aria-labelledby="intelligence-health-title">
      ...
      {viewMode === "executive" && <section className="mission-control executive-mission-control" aria-label="Executive Brief">
        ...
        <article className="stat-card wide-card"><h3>Warning Verification Center</h3><p><strong>Release answer:</strong> Are we broken? No. Are we regressing? No. Are we ready to release? {warningCount ? "Not yet — operational warnings need verification." : "Ready after gates pass."}</p><p>{warningVerificationCenter.summary || `${warningCount} operational warnings require verification before release readiness approval.`}</p>{warningVerificationItems.length ? warningVerificationItems.map((item, index) => <section className="stat-card" key={`${item.layer || "warning"}-${index}`}><h4>{item.layer || `Warning ${index + 1}`} · {item.warning_type || "operational warning verification"}</h4><p><strong>Current status:</strong> {item.current_status || "WARNING"}</p><p><strong>Why still a warning:</strong> {item.why_it_is_still_a_warning || "Missing warning evidence; degraded gracefully until runtime evidence is attached."}</p><p><strong>Evidence required to clear:</strong> {item.evidence_required_to_clear || "Runtime evidence, baseline comparison, no-regression confirmation, and rerun proof are required."}</p><p><strong>Owner:</strong> {item.owner || "Platform intelligence owner · evidence review"}</p><p><strong>Expected cleared state:</strong> {item.expected_cleared_state || "Warning verified with evidence and no longer blocking release readiness."}</p><p><strong>Release readiness impact:</strong> {item.release_readiness_impact || "Release Readiness remains below ready until this gate clears."}</p><p><strong>Baseline review required:</strong> {item.baseline_review_required ? "Yes" : "No"} · <strong>Rerun required:</strong> {item.rerun_required ? "Yes" : "No"} · <strong>Code change required:</strong> {item.code_change_required ? "Yes" : "No"}</p><p><strong>Latest diagnostic ran:</strong> {item.latest_diagnostic_ran ? "Yes" : "No"}</p><p><strong>Expected upstream payload:</strong> {formatList(item.upstream_payload_required)}</p><p><strong>Expected downstream payload:</strong> {item.downstream_payload_expected || "Downstream payload proof required."}</p><p><strong>Runtime evidence required:</strong> {item.runtime_evidence_required || "Layer ran, consumed upstream payload, produced downstream payload, and modified no production records."}</p><p><strong>Baseline mismatch review:</strong> {item.baseline_mismatch_review || "still_unexplained_until_review"}</p><p><strong>No-regression proof:</strong> {item.no_regression_confirmation || "regression_count remains 0 and no root-cause layer is selected."}</p><p><strong>Production record safety:</strong> {item.production_record_safety || "Mission Control remains read-only."}</p><p><strong>Mark Verified rule:</strong> {item.mark_verified_rule || "Cannot mark verified until evidence gates pass."}</p><p><strong>Specific verification actions:</strong></p><ol>{asArray(item.verification_action).length ? asArray(item.verification_action).map((action) => <li key={action}>{action}</li>) : ["Review runtime evidence", "Compare baseline", "Confirm no regression", "Rerun diagnostic", "Mark warning verified only if evidence passes"].map((action) => <li key={action}>{action}</li>)}</ol><p><strong>Next action:</strong> {item.next_action || "Review runtime evidence → Compare baseline → Confirm no regression → Rerun diagnostic → Mark warning verified only if evidence passes"}</p><div className="action-strip compact"><button type="button" onClick={showRootCauseEvidence}>View Evidence</button><button type="button" onClick={showRootCauseEvidence}>Compare Baseline</button><button type="button" onClick={run} disabled={actionDisabled}>Launch Verification</button><button type="button" disabled title={item.mark_verified_rule || "Evidence gates must pass before verification can be marked."}>Mark Verified</button></div></section>) : <p>Missing warning evidence degrades gracefully: run diagnostics to attach owner, evidence requirements, and next actions.</p>}</article>
        <article className="stat-card wide-card"><h3>Release Readiness Checklist</h3><table className="admin-table"><thead><tr><th>Gate</th><th>Status</th><th>Evidence</th></tr></thead><tbody>{(releaseReadinessChecklist.length ? releaseReadinessChecklist : [{ gate: "Regression Gate", status: regressionCount ? "FAIL" : "PASS", evidence: "Zero regressions means no root-cause layer is selected." }, { gate: "Runtime Evidence Gate", status: "PENDING", evidence: "Runtime evidence must be reviewed." }, { gate: "Warning Verification Gate", status: warningCount ? "PENDING" : "PASS", evidence: "Warnings must be verified with evidence." }, { gate: "Baseline Review Gate", status: warningCount ? "PENDING" : "PASS", evidence: "Do not update baselines unless intentionally obsolete." }, { gate: "Public Report Gate", status: safePublicReportUrl ? "PASS" : "PENDING", evidence: "Public report must be generated and verified." }, { gate: "Read-only Safety Gate", status: result?.production_writes === 0 && !result?.workflow_execution ? "PASS" : "FAIL", evidence: "Dashboard remains read-only." }, { gate: "Deployment Evidence Gate", status: deploymentEvidenceVerified ? "PASS" : "PENDING", evidence: "Deployment evidence must verify runtime propagation." }, { gate: "Discord Config Gate", status: discordConfigurationWarnings.length ? "PENDING" : "PASS", evidence: "Discord webhook/config health is separate from SimbaBrain intelligence health." }]).map((gate) => <tr key={gate.gate}><td>{gate.gate}</td><td>{gate.status}</td><td>{gate.evidence}</td></tr>)}</tbody></table></article>
      ...
      <article className="stat-card wide-card" aria-label="Executive Work Queue"><h4>Executive Work Queue</h4><table className="admin-table"><thead><tr><th>Priority</th><th>Work Item</th><th>Business Value</th><th>Estimated Time</th><th>Dependencies</th><th>Expected Metric Improvement</th><th>Responsible Intelligence Layer</th><th>Recommended Next Action</th></tr></thead><tbody>{executiveWorkQueue.map((item) => <tr key={item.id}><td><strong>{item.priorityLabel}</strong></td><td>{item.title}</td><td>{item.businessValue}</td><td>{item.estimatedTime}</td><td>{item.dependencies}</td><td>{item.expectedMetricImprovement}</td><td>{item.responsibleLayer}</td><td><strong>{item.recommendedNextAction}</strong><div className="action-strip compact">{item.executableActions.map((action) => <button type="button" key={action} onClick={action === "Launch Verification" ? run : action === "View Evidence" || action === "Compare Baseline" ? showRootCauseEvidence : undefined} disabled={actionDisabled && action === "Launch Verification"}>{action}</button>)}</div></td></tr>)}</tbody></table></article>

      <h3>Executive Decision Engine</h3>
      <div className="dashboard-grid decision-card-grid">{executiveDecisionCards.map((card) => <article className="stat-card decision-card" key={card.id}><h4>{card.decision}</h4><p><strong>Decision Type:</strong> {card.decisionType}</p><p><strong>Reason:</strong> {card.reason}</p><p><strong>Expected Benefit:</strong> {card.expectedBenefit}</p><p><strong>Risk if Deferred:</strong> {card.riskIfDeferred}</p><p><strong>Confidence:</strong> {card.confidence}</p><p><strong>Owner:</strong> {card.owner}</p><p><strong>Estimated Completion:</strong> {card.estimatedCompletion}</p><div className="action-strip compact">{card.executableActions.map((action) => <button type="button" key={action} onClick={action === "Launch Verification" ? run : action === "View Evidence" || action === "Compare Baseline" ? showRootCauseEvidence : undefined} disabled={actionDisabled && action === "Launch Verification"}>{action}</button>)}</div></article>)}</div>
