...
export default function IntelligenceHealthMonitor() {
  ...
  const rootCauseSummary = rootCauseLayer ? `One upstream regression is affecting ${blastRadius.length} downstream systems.` : "No upstream regression is currently affecting downstream systems.";
  const downstreamSystemsSummary = blastRadius.length ? blastRadius.map(operationalLayerName).join(", ") : "No downstream systems affected.";
  ...
  const priorityQueue = rootCauseLayer ? [
  ...
  const executiveTimelineLabels = ["Diagnostic Started", "Evidence Generated", "Reports Generated", "Verification Complete", "Results Published"];
  const executiveTimeline = executiveTimelineLabels.map((label, index) => {
    const source = timeline[index] || {};
    return { time: source.time || source.timestamp || `09:${String(26 + index).padStart(2, "0")}`, label, status: source.status || (index <= 1 || hasDiagnosticResult ? "Complete" : "Pending") };
  });
  ...
  const downstreamLayerNames = blastRadius.map(operationalLayerName);
  const rootCauseStatus = rootCauseLayer ? (regressionCount ? "Regression" : failureCount ? "Failure" : "Warning") : "Stable";
  const rootCauseBusinessImpact = rootCauseLayer ? (blastRadius.length >= 5 ? "High" : "Medium") : "Low";
  const rootCauseReason = rootCauseClassification.reason || whyThisMatters.what_changed || result?.executive_summary || "Priority logic changed during verification.";
  const rootCauseFix = rootCauseLayer ? (priorityQueue[0]?.title || "Review priority calculation.") : "Continue monitoring verified evidence.";
  const affectedSystemsRows = blastRadius.map((layer) => {
    const match = layers.find((item) => (item.layer || "").includes(layer));
    const evidence = runtimeEvidenceByLayer.get(layer);
    return { layer: operationalLayerName(layer), status: executiveStatusFor(match, evidence), impact: ["Decision Support", "Execution Planning", "Execution Intelligence"].includes(layer) ? "High" : "Medium", blockedBy: rootCauseLayer ? operationalLayerName(rootCauseLayer) : "No blocker detected" };
  });
  const actionCenterCommands = [["Investigate Root Cause", undefined], ["Run Verification", run], ["Open Evidence", undefined], ["Generate Executive Report", generateReport], ["Generate Board Report", generateReport], ["Prepare Release", undefined]];
  const metricHistoryPoints = asArray(trends.overall_health_score).length ? asArray(trends.overall_health_score) : history.slice(0, 12).reverse().map((run) => ({ timestamp: run.created_at, value: getRunHealth(run) })).filter((point) => point.value != null);
  const metricHistoryPrevious = getRunHealth(previousRun) ?? getRunHealth(result);
  const metricHistoryTrend = trendDirectionFrom(getRunHealth(result), metricHistoryPrevious);
  const metricHistoryMax = Math.max(1, ...metricHistoryPoints.map((point) => Number(point.value) || 0), Number(healthScore) || 0);
  ...
  return (
    <section className="cosmic-section intelligence-health-monitor" aria-labelledby="intelligence-health-title">
      ...
      {viewMode === "executive" && <section className="mission-control executive-mission-control" aria-label="Executive Brief">
        <article className="stat-card wide-card executive-summary-card" aria-label="Executive Summary">
          <p className="section-kicker">Executive Operating System</p>
          <h3>Mission Control Command Summary</h3>
          <div className="executive-focus-grid">
            {[ ["Platform Health", `${healthScore}%`], ["Production Status", commandCenter.deployment_status || displayedPipelineOverallStatus], ["Root Cause", rootCauseLayer ? operationalLayerName(rootCauseLayer) : "No active root cause"], ["Affected Systems", downstreamLayerNames.length ? `${downstreamLayerNames.length} systems` : "None"], ["Estimated Time To Recovery", timeToResolution], ["Executive Confidence", `${dailyBriefing.confidence}%`] ].map(([label, value]) => <article className="focus-tile" key={label}><span>{label}</span><strong>{value}</strong></article>)}
          </div>
          <p>{rootCauseLayer ? `${operationalLayerName(rootCauseLayer)} is driving the current operating risk.` : "No active upstream break is driving the system."} Leadership should resolve the first blocker before trusting downstream planning.</p>
        </article>

        <div className="dashboard-grid executive-command-overview">
          <article className="stat-card executive-priority" aria-label="Root Cause"><h3>ROOT CAUSE</h3><h2>{rootCauseLayer ? operationalLayerName(rootCauseLayer) : "None Detected"}</h2><p><strong>Status:</strong> {rootCauseStatus}</p><p><strong>Business Impact:</strong> {rootCauseBusinessImpact}</p><p><strong>Downstream Systems:</strong> {blastRadius.length}</p><p><strong>Reason:</strong> {rootCauseReason}</p><p><strong>Recommended Fix:</strong> {rootCauseFix}</p><p><strong>Estimated Recovery:</strong> {timeToResolution}</p></article>
          <article className="stat-card" aria-label="What Should I Do First"><h3>What should I do first?</h3><p>{priorityQueue[0]?.title || highestPriority}</p><p>Start with the Root Cause card. Downstream systems should be verified after that blocker is addressed.</p></article>
          <article className="stat-card" aria-label="What Happens If Ignored"><h3>If ignored</h3><p>{decisionCards[0]?.risks || "Downstream recommendations may stay unreliable."}</p><p>Leadership confidence remains limited until the root cause is reviewed.</p></article>
        </div>

        <article className="stat-card wide-card" aria-label="Affected Systems"><h3>Affected Systems</h3><table className="admin-table"><thead><tr><th>System</th><th>Current Status</th><th>Business Impact</th><th>Blocked By</th></tr></thead><tbody>{affectedSystemsRows.length ? affectedSystemsRows.map((row) => <tr key={row.layer}><td>{row.layer}</td><td>{row.status}</td><td>{row.impact}</td><td>{row.blockedBy}</td></tr>) : <tr><td colSpan={4}>No downstream systems are currently blocked.</td></tr>}</tbody></table></article>

        <article className="stat-card wide-card" aria-label="Executive Actions"><h3>Executive Actions</h3><div className="action-strip executive-command-strip">{actionCenterCommands.map(([command, handler]) => <button key={command} type="button" onClick={handler} disabled={actionDisabled && ["Run Verification", "Generate Executive Report", "Generate Board Report"].includes(command)}>{command}</button>)}</div></article>

        <article className="stat-card wide-card" aria-label="Metric History"><h3>Metric History</h3><div className="dashboard-grid"><section className="stat-card"><h4>Overall Health</h4><p><strong>Current:</strong> {formatMetric(getRunHealth(result), "%")}</p><p><strong>Previous:</strong> {formatMetric(metricHistoryPrevious, "%")}</p><p><strong>Trend:</strong> {metricHistoryTrend === "improved" ? "Improving" : metricHistoryTrend === "worse" ? "Declining" : "Stable"}</p><div className="spark-bars" aria-label="Overall Health history">{(metricHistoryPoints.length ? metricHistoryPoints : [{ timestamp: diagnosticTimestamp, value: getRunHealth(result) }]).map((point, index) => <span key={`${point.timestamp || "current"}-${index}`} title={`${point.timestamp || "Current"}: ${point.value}%`} style={{ height: `${Math.max(4, ((Number(point.value) || 0) / metricHistoryMax) * 48)}px` }} />)}</div></section></div></article>

        <h3>Executive Timeline</h3>
        <article className="stat-card wide-card executive-timeline">{executiveTimeline.map((event) => <div className="timeline-row" key={`${event.time}-${event.label}`}><time>{event.time}</time><span>{event.label}</span><strong>{event.status}</strong></div>)}</article>
      </section>}
      ...
      <div className="dashboard-grid daily-agenda-grid">{dailyAgenda.map((item, index) => <article className="stat-card" key={item.title}><h4>{index + 1}. {item.title}</h4><p><strong>Estimated Time:</strong> {item.time}</p><p><strong>Priority:</strong> {item.priority}</p></article>)}</div>

      <h3>AI COO Decision Center</h3>
      <div className="dashboard-grid decision-card-grid">{decisionCards.map((card) => <article className="stat-card decision-card" key={card.id}><h4>{card.title}</h4><p><strong>Current Status:</strong> {card.status}</p><p><strong>Why this matters:</strong> {card.why}</p><p><strong>Business Impact:</strong> {card.impact}</p><p><strong>Estimated Effort:</strong> {card.effort}</p><p><strong>Expected Improvement:</strong> {card.improvement}</p><p><strong>Dependencies:</strong> {card.dependencies}</p><p><strong>Risks:</strong> {card.risks}</p><p><strong>Recommended Owner:</strong> {card.owner}</p><p><strong>Recommended Completion Date:</strong> {card.deadline}</p></article>)}</div>

      <h3>Smart Prioritization · Highest ROI First</h3>
      <article className="stat-card wide-card"><table className="admin-table"><thead><tr><th>Rank</th><th>Highest Return Item</th><th>Expected Gain</th><th>Time</th><th>Complexity</th><th>Command</th></tr></thead><tbody>{decisionCards.map((card, index) => <tr key={card.id}><td>{index + 1}</td><td>{card.title}</td><td>{card.improvement}</td><td>{card.effort}</td><td>{index === 0 ? "Medium Complexity" : "Low Complexity"}</td><td>{card.command}</td></tr>)}</tbody></table></article>
      ...
      <article className="stat-card wide-card dependency-map mission-pipeline" aria-label="Visual Intelligence Pipeline">
        ...
        <p><strong>Root cause:</strong> {rootCauseLayer ? `${operationalLayerName(rootCauseLayer)} is the first point of failure.` : "No broken propagation detected."}</p>
      ...
      <div className="dashboard-grid executive-layer-grid">{dependencyLayers.map((layer, index) => {
        ...
        return <article className={`stat-card executive-layer-card status-${status.toLowerCase()}`} key={layer}><h4>{executiveStatusIcon(status)} {operationalLayerName(layer)}</h4><p><strong>Role:</strong> {index === firstFailureIndex ? "Root cause / first point of failure" : index > firstFailureIndex && firstFailureIndex >= 0 ? "Affected downstream impact" : "Upstream context"}</p><p><strong>Status:</strong> {status}</p><p><strong>Why:</strong> {status === "Connected" ? (match?.plain_language_reason || executiveConnectionPhrase(layer, nextLayer)) : isRuntimeVerified(evidence) ? executiveConnectionPhrase(layer, nextLayer) : (match?.explanation || "Information has not been proven to reach the next intelligence layer yet.")}</p><p><strong>Business impact:</strong> {status === "Connected" ? "Leadership can rely on this PASS layer when making decisions." : "Repair this handoff before using downstream recommendations for final decisions."}</p><p><strong>Affected systems:</strong> {dependencyLayers.slice(index + 1, Math.min(dependencyLayers.length, index + 4)).map(operationalLayerName).join(", ") || "Final intelligence destination"}</p><p><strong>Recommended fix:</strong> {match?.suggested_admin_action || action?.title || "Assign an owner, repair the handoff, and rerun Mission Control."}</p><p><strong>Expected improvement:</strong> {executiveDiagnosticCopy(action?.improvement, hasDiagnosticResult ? "Improvement will be calculated after verification." : "Not measured until a diagnostic run exists.")}</p><p><strong>Estimated effort:</strong> {action?.effort || timeToResolution}</p><p><strong>Confidence:</strong> {match?.confidence ?? action?.confidence ?? dailyBriefing.confidence}%</p></article>;
      })}</div>

      <h3>Executive Trends</h3>
      <article className="stat-card wide-card"><label htmlFor="trend-window"><strong>View window</strong></label> <select id="trend-window" value={trendWindow} onChange={(event) => setTrendWindow(event.target.value)}><option value="10">Last 10 runs</option><option value="30">Last 30 runs</option><option value="100">Last 100 runs</option><option value="all">All Time</option></select><div className="dashboard-grid">{trendMetrics.filter(([, key]) => ["overall_health_score", "regression_count", "api_latency_ms", "diagnostic_duration_ms", "public_verification_score", "public_verification_latency_ms"].includes(key)).map(([label, key, unit]) => { const points = asArray(trends[key]); const last = points[points.length - 1]; const max = Math.max(1, ...points.map((point) => Number(point.value) || 0)); return <section className="stat-card" key={key}><h4>{label === "Overall Health Score" ? "Health trend" : label === "Regression Count" ? "Regression trend" : label === "API Latency" ? "API latency" : label === "Diagnostic Duration" ? "Diagnostic duration" : label === "Public Verification Latency" ? "Public verification latency" : "Public verification result"}</h4><p>{last?.value ?? "—"}{unit}</p><div className="spark-bars" aria-label={`${label} chart`}>{points.map((point, index) => <span key={`${key}-${index}`} title={`${point.timestamp}: ${point.value}${unit}`} style={{ height: `${Math.max(4, ((Number(point.value) || 0) / max) * 48)}px` }} />)}</div><div className="action-strip compact"><button type="button">View Evidence</button><button type="button">Investigate</button></div></section>; })}<section className="stat-card"><h4>Institutional Memory</h4><p>Average health {formatMetric(historyStats.averageHealth, "%")} · Average deployment quality {formatMetric(historyStats.averageDeploymentQuality, "%")}</p><p>Most common failures: {historyStats.mostCommonFailures}</p><p>Most unstable layer: {historyStats.mostUnstableLayer}</p><p>Fastest improving layer: {historyStats.fastestImprovingLayer}</p><p>Average verification result {formatMetric(historyStats.averageVerificationScore, "%")} · Average diagnostic duration {formatMetric(historyStats.averageDiagnosticDuration, "ms")}</p><div className="action-strip compact"><button type="button">Open Layer</button><button type="button">View Evidence</button></div></section><section className="stat-card"><h4>Deployment history</h4><p>{history.length || "—"} recorded runs</p><div className="spark-bars" aria-label="Deployment history chart">{history.slice(0, 12).reverse().map((run, index) => <span key={run.diagnostic_id || index} title={run.created_at} style={{ height: `${Math.max(4, Number(run.overall_health_percent ?? run.overall_health?.percent ?? 0) / 2)}px`, background: "#38bdf8" }} />)}</div></section></div></article>
      ...
      <div className="ecosystem-command-grid">{ecosystemCommandSystems.map((system) => <article className="stat-card ecosystem-command-card" key={system.subsystem}><h4>{system.subsystem}</h4><p><strong>Health:</strong> {system.health ?? "—"}%</p><p><strong>Trend:</strong> {system.performance || "Stable"}</p><p><strong>Risk:</strong> {asArray(system.warnings).join(", ") || "Low"}</p><p><strong>Recommendation:</strong> {asArray(system.recommendations).join("; ") || "Continue monitoring"}</p></article>)}</div>
