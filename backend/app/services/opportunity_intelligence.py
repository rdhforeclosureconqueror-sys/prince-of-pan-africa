...
def generate_opportunity_intelligence(db: Session, *, society_id: int | None = None, include_debug: bool = False) -> dict[str, Any]:
    ...
    for society in societies:
        ...
        institution_financial_readiness = ii["scores"]["financial_readiness"]["score"]
        institution_score = ii["institution_health"]["score"]
        operational_score = ii["scores"]["operational_readiness"]["score"]
        institution_backed_business_ready = institution_score >= 60 and operational_score >= 70
        if scores["business_readiness"]["score"] >= 60 or institution_financial_readiness >= 70 or institution_backed_business_ready:
            business_confidence = max(scores["business_readiness"]["score"], institution_financial_readiness, institution_score if institution_backed_business_ready else 0)
            business_evidence = scores["business_readiness"].get("evidence", [])
            if institution_financial_readiness >= 70:
                business_evidence += ii["scores"]["financial_readiness"].get("evidence", [])
            if institution_backed_business_ready:
                business_evidence += ii["institution_health"].get("evidence", []) + ii["scores"]["operational_readiness"].get("evidence", [])
            opportunities.append(_opp(oid=f"society-{society.id}-business-incubation", title="Evaluate business incubation readiness", typ="business", parts={"impact": 90, "confidence": business_confidence, "trust": scores["trust_score"]["score"], "leadership": scores["leadership_coverage"]["score"], "effort": 70}, action="Convene manual business incubation review.", reason="Business readiness, trust, and leadership evidence suggest a possible incubation opportunity. Institution financial readiness can qualify this recommendation when Society business readiness alone is still developing.", evidence=business_evidence, missing=scores["business_readiness"].get("missing_evidence", []), members=[p.user_id for p in profiles if p.current_projects_json], societies=[society.id]))
        ...
        if institution_score >= 60 and operational_score >= 70:
            opportunities.append(_opp(oid=f"society-{society.id}-institution-formation", title="Consider institution formation", typ="institution", parts={"impact": 95, "confidence": institution_score, "readiness": operational_score, "effort": 80}, action="Manually review institution formation readiness.", reason="Institution Intelligence is stable-but-developing and operational readiness is strong enough for human institution formation review.", evidence=ii["institution_health"].get("evidence", []) + ii["scores"]["operational_readiness"].get("evidence", []), missing=ii.get("missing_evidence", []), societies=[society.id], institutions=[society.id]))
