...
from app.services.predictive_intelligence import generate_predictive_intelligence
...
@router.get("/predictive/intelligence")
def predictive_intelligence(society_id: int | None = None, debug: bool = False, current_user: User = Depends(require_permission("society_builder:read_admin")), db: Session = Depends(get_db)):
    require_society_builder_enabled(db, current_user)
    include_debug = debug and user_has_permission(db, current_user, "society_builder:read_admin")
    try:
        return generate_predictive_intelligence(db, society_id=society_id, include_debug=include_debug)
    except ValueError:
        raise HTTPException(status_code=404, detail="Society not found")
