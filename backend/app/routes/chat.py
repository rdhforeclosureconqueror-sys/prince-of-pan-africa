...
def openai_response(prompt: str) -> str:
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Backend failed to generate response: {e}")


@router.get("/ping")
async def ping_mufasa():
    return {"status": "ok", "message": "Prince of Pan-Africa backend is online."}
...
async def generate_openai_response(payload: dict):
    ...
    return {
        ...
        "audio_url": audio_url,
        "voice": voice_model,
        "source": "PrinceOfPanAfricaBackend",
    }
